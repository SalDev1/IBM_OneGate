...
export default function AmenityDate() {
  ...
  const [value, setValue] = useState<Dayjs | null>(
    blackoutDates.includes(dayjs(new Date()).toISOString().split("T")[0])
      ? null
      : dayjs(new Date())
  );
  ...
  const [monthYear, setMonthYear] = useState<{
    ...
    month: Number(dayjs(new Date()).month()) + 1,
    year: Number(dayjs(new Date()).year()),
  });
  ...
  const handleChange = async (event: React.ChangeEvent) => {
    if (
      ...
      Number((event.target as HTMLInputElement).value) <=
        (5 < amenity.Limit ? 5 : amenity.Limit)
    ) {
  ...
  const getBlackoutDates = useCallback(async () => {
    const response = await fetch("http://localhost:4000/bookings", {
      ...
      body: JSON.stringify({
        ...
        monthNo: monthYear.month,
        yearNo: monthYear.year,
      }),
    });
